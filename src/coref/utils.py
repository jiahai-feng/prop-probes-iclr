...
def slugify(value, allow_unicode=False):
    ...
    underscores, or hyphens to underscores. Convert to lowercase. Also strip
    leading and trailing whitespace, dashes, and underscores.

    Changed on Feb 12, 2024: illegal characters are converted to _ instead of
    removed.
    """
    value = str(value)
    if allow_unicode:
...
def cache(name, thing, prefixes, force_compute=False, read_only=False):
    ...
    if not force_compute and os.path.exists(file_path):
        ...
        if read_only:
            raise Exception(
                "force_compute and read_only cannot both be true."
                if force_compute
                else f"Cannot find cached file {file_path}"
            )
